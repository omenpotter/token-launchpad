...
import * as X1RpcService from './X1RpcService';
...
export async function checkX1OnChainLiquidity(tokenMint) {
  try {
    ...
    try {
      const recentBlocks = await X1RpcService.getRecentBlocks(5);
      const hasActivity = recentBlocks && recentBlocks.length > 0;
      ...
      return {
        ...
        blockHeight: recentBlocks?.[0]?.blockHeight || 'unknown',
        detectedAt: new Date().toISOString()
      };
    } catch (activityError) {
...
export async function getTokenHolderCount(tokenMint) {
  try {
    ...
    // Get multiple accounts holding this token
    const accounts = await X1RpcService.getMultipleAccounts([tokenMint]);
...
export async function getX1TokenSupply(tokenMint) {
  try {
    ...
    // Mint decimals at offset 44
    const decimals = data[44];
    
    // Total supply at offset 36-44 (little-endian u64)
    ...
    const supply = Number(supplyBigInt) / Math.pow(10, decimals);

    return {
      supply: supply,
      decimals: decimals,
      supplyRaw: supplyBigInt.toString()
    };
  } catch (error) {
